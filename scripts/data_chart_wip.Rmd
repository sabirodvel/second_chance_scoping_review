...
map_facility_level <- function(x) {
  ...
  case_when(
    ...
    # Secondary 
    str_detect(x_norm, "\\b(district|municipal|county)\\s+hospital\\b") ~ "Secondary",
    str_detect(x_norm, "\\bgeneral\\s+hospital\\b") ~ "Secondary",
    str_detect(x_norm, "\\b(regional|provincial)\\s+hospital\\b") ~ "Secondary",
    
    # Tertiary
    str_detect(x_norm, "\\b(referral|teaching|specialized|university)\\s+hospital\\b") ~ "Tertiary",
    str_detect(x_norm, "\\bnational\\s+hospital\\b|referral|refferal|national") ~ "Tertiary",
...
derive_facility_type <- function(...) {
  ...
  # Detect each bucket (map synonyms -> 3 classes)
  ...
  is_secondary <- grepl("\\b(district|municipal|county|sub[- ]?county)\\s+hospital\\b|
                       \\bgeneral\\s+hospital\\b|\\bsecondary\\b|\\b(regional|provincial)\\s+hospital\\b|
                       \\bregional\\b", corpus)
  is_tertiary <- grepl("\\b(referral|refferal|tertiar(y|ies)|tertiary|teaching|university|
...
extract_facility_counts <- function(text) {
  ...
  national <- ex("(\\d[\\d,\\.]*)\\s+national\\s+hospitals?")

  typed_sum <- sum(c(primary, district, regional, national), na.rm = TRUE)

  total <- if (is.finite(typed_sum) && typed_sum > 0) {
    typed_sum
  } else {
...
mini_final_cleaned <- tibble::tibble(
  ...
  `Number of National Hospitals` = fac_counts$national,
