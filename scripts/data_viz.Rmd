...
filtered_consensus <- extracted_studies_may_12 %>%
...
geo_distribution <- ggplot(data = africa_data) +
  ...
  study_fill_scale +
  labs(title = "Distribution of Studies on Plastic Reconstructive Surgery in Sub-Saharan Africa",
       subtitle = "Countries in grey have been excluded from the mandate.",
       caption = "Source: Covidence 'Data Charting' Dataset") +
  theme_minimal() +
  theme(legend.position = "top",
        axis.text = element_blank(), 
...
# Save plot as image
ggsave(filename = here("images/geo_distribution_may_12.png"), plot = combined_choro_bar, height = 10, width = 13, bg = "white")
...
# Save image
# ggsave(filename = here("images/surgeries_lollipop.png"), bg = "white", plot = lollipop, width = 7, height = 4, dpi = 300)
```
...
daly_data <- pathology_daly %>%
  mutate(
    general_category_of_pathology =
      case_when(
        general_category_of_pathology == "general pediatric surgery pathology" |
          general_category_of_pathology == "pediatric surgical cases" ~ "general surgery",
